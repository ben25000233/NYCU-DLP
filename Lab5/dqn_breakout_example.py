...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        ## TODO ##
        if random.random() < epsilon:  # random choose
            return action_space.sample()
        else: 
            with torch.no_grad():
                state = np.array(state)
                t_state = self._behavior_net.forward(torch.from_numpy(state).unsqueeze(0).permute(0,3,1,2).to(self.device))
                action = torch.argmax(t_state).item()
            return action
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            ...
            action = torch.argmax(q_next, dim=1)
            q_next = torch.gather(q_next, 1, action.view([self.batch_size, 1]).long())
            q_target = reward + gamma*q_next*(1-done) #by bellman
...
def main():
    ...
    parser.add_argument('--logdir', default='log/dqn_v4')
    # train
